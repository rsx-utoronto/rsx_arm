...
class ArmSciNode(Node):
    def __init__(self):
        ...
        self.arm = SciArm(5, dhTable, offsets,
                          angleOrientation, startingAngles)
        self.sparkMaxOfssets = [0]*len(angleOrientation)

        # Publishers
        self.goalPub = self.create_publisher(
            Float32MultiArray, "arm_goal_pos", 10)
        self.vizPub = self.create_publisher(
            Float32MultiArray, "arm_viz_pos", 10)

        # Subscribers
        self.create_subscription(
            String, "arm_state", self.onArmStateUpdate, 10)
        self.create_subscription(
            ArmInputs, "arm_inputs", self.onJoystickUpdate, 10)
        self.create_subscription(
            Float32MultiArray, "arm_curr_pos", self.onCurrPosUpdate, 10)

        # Timer for main loop
        # main function inside this class will be called at initialization
        self.create_timer(1.0 / 30, self.main)
    ...
    def handle_custom_sampling(self):
        ...
        success, message = self.arm.execute_custom_sequence(
            checkpoints, speed_factor=0.5)
    ...
    def publishAngles(self, anglesToPub):
        ...
        correctedDirection = self.arm.correctAngleDirection(
            self.arm.getPublishingAngles())
    ...
    def onCurrPosUpdate(self, data: Float32MultiArray):
        ...
        offsetsRemovedAngles = self.arm.removeSparkMaxOffsets(
            deg2rad(correctedOrientation))
    ...
    def onJoystickUpdate(self, data):
        ...
        buttonsPressed = [data.x, data.o, data.triangle, data.square, data.l1,
                          data.r1, data.l2, data.r2, data.share, data.options, 0, 0, 0, 0, 0, 0, 0]
