...
def anglePosition():
    ...
    angleArray = list(
        map(float, [angle1, angle2, angle3, angle4, angle5, angle6]))
    for i in range(len(angleArray)):
        angleArray[i] = angleArray[i] * math.pi / \
            180  # Conversion from degrees to radians
    return angleArray
...
def framePosition():
    ...
    posArray = [tempArray[0], tempArray[1], tempArray[2],
...
class ArmVisualizationNode(Node):
    ...
    def __init__(self):
        ...
        self.gazebo_on = self.get_parameter(
            'gazebo_on').get_parameter_value().bool_value
        ...
        if self.gazebo_on:
            ...
            self.jointPublisher = self.create_publisher(
                JointState, "joint_states", 10)

        self.create_subscription(String, "arm_state", self.updateState, 10)
        self.create_subscription(
            Float32MultiArray, "arm_viz_pos", self.displayArmGoalPos, 10)
        ...
        try:
            if self.tf_buffer.can_transform(referenceFrame, targetFrame, Time()):
                trans = self.tf_buffer.lookup_transform(
                    referenceFrame, targetFrame, Time())
    ...
    def runNewDesiredJointState(self, angles):
        ...
        newJointState.name = ["joint_1", "joint_2", "joint_3", "joint_4",
                              "joint_5", "joint_6", "finger_joint_1", "finger_joint_2", "camera_joint"]
    ...
    def runNewSciJointState(self, angles):
        ...
        newJointState.header = Header()
        newJointState.header.stamp = self.get_clock().now().to_msg()
        newJointState.name = ["Joint_1", "Joint_2",
    ...
    def displayArmGoalPos(self, data):
        ...
        tempAngles = list(subtract(array(tempAngles), -
                          1*array(self.savedSciCanAngles)))
    ...
    def displayArmLivePos(self, data):
        ...
        if not self.gazebo_on:
            ...
            tempAngles = list(
                subtract(array(tempList), array(self.savedCanAngles)))
