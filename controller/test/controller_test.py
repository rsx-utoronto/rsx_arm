...
class test_node(Node):
    def __init__(controller_node, publishers, subscribers):
        ...
        for publisher in publishers:
            controller_node.test_publishers[publisher[0]] = controller_node.create_publisher(
                publisher[1], publisher[2], 10)

        controller_node.test_subscribers = {}
        for subscriber in subscribers:
            controller_node.test_subscribers[subscriber[0]] = controller_node.create_subscription(
                subscriber[1], subscriber[2], controller_node.listener_callback, 10)
