from enum import Enum


class ArmState(Enum):
    IDLE = 0
    MANUAL = 1
    IK = 2
    PATH_PLANNING = 3
    SCIENCE = 4
    AUTO_KEYBOARD = 5


class ControlMode(Enum):
    CONTROLLER = 0
    KEYBOARD = 1
    GUI = 2


class HomingStatus(Enum):
    IDLE = 0
    ACTIVE = 1
    COMPLETE = 2


class SafetyErrors(Enum):
    NONE = 0
    EXCEEDING_POS = 1
    EXCEEDING_CURRENT = 2
    EXCEEDING_POS_CURRENT = 3
    LIMIT_SWITCH = 4  # May switch out of an error as this is not only used as an error


class CANAPI(Enum):
    # CANSpark APIs
    CMD_API_SETPNT_SET = 0x001
    CMD_API_DC_SET = 0x002
    CMD_API_SPD_SET = 0x012
    CMD_API_SMART_VEL_SET = 0x013
    CMD_API_POS_SET = 0x032
    CMD_API_VOLT_SET = 0x042
    CMD_API_CURRENT_SET = 0x043
    CMD_API_SMARTMOTION_SET = 0x052
    CMD_API_STAT0 = 0x060
    CMD_API_STAT1 = 0x061
    CMD_API_STAT2 = 0x062
    CMD_API_STAT3 = 0x063
    CMD_API_STAT4 = 0x064
    CMD_API_STAT5 = 0x065
    CMD_API_STAT6 = 0x066
    CMD_API_CLEAR_FAULTS = 0x06E
    CMD_API_DRV_STAT = 0x06A
    CMD_API_BURN_FLASH = 0x072
    CMD_API_SET_FOLLOWER = 0x073
    CMD_API_FACTORY_DEFAULT = 0x074
    CMD_API_FACTORY_RESET = 0x075
    CMD_API_IDENTIFY = 0x076
    CMD_API_NACK = 0x080
    CMD_API_ACK = 0x081
    CMD_API_BROADCAST = 0x090
    CMD_API_HEARTBEAT = 0x092
    CMD_API_SYNC = 0x093
    CMD_API_ID_QUERY = 0x094
    CMD_API_ID_ASSIGN = 0x095
    CMD_API_FIRMWARE = 0x098
    CMD_API_ENUM = 0x099
    CMD_API_LOCK = 0x09B
    CMD_API_LOCKB = 0x0B1
    CMD_API_NONRIO_HB = 0x0B2

    CMD_API_SWDL_BOOTLOADER = 0x1FF
    CMD_API_SWDL_DATA = 0x09C
    CMD_API_SWDL_CHKSUM = 0x09D
    CMD_API_SWDL_RETRANSMIT = 0x09E

    CMD_API_MECH_POS = 0x0A0
    CMD_API_I_ACCUM = 0x0A2
    CMD_API_ANALOG_POS = 0x0A3
    CMD_API_ALT_ENC_POS = 0x0A4
    CMD_API_PARAM_ACCESS = 0x300

class ODRIVE_CANAPI(Enum):
    # ODrive APIs
    CMD_API_GET_VERSION = 0x000
    CMD_API_HEARTBEAT = 0x001
    CMD_API_ESTOP = 0x002
    CMD_API_GET_ERROR = 0x003
    CMD_API_RXSDO = 0x004
    CMD_API_TXSDO = 0x005
    CMD_API_ADDRESS = 0x006
    CMD_API_SET_AXIS_STATE = 0x007
    CMD_API_GET_ENCODER_ESTIMATES = 0x009
    CMD_API_GET_ENCODER_COUNT = 0x00a
    CMD_API_SET_CONTROLLER_MODE = 0x00b
    CMD_API_SET_INPUT_POS = 0x00c
    CMD_API_SET_INPUT_VEL = 0x00d
    CMD_API_SET_INPUT_TORQUE = 0x00e
    CMD_API_SET_LIMITS = 0x00f
    CMD_API_SET_TRAJ_VEL_LIMIT = 0x011
    CMD_API_SET_TRAJ_ACCEL_LIMITS = 0x012
    CMD_API_SET_TRAJ_INTERTIA = 0x013
    CMD_API_GET_IQ = 0x014
    CMD_API_GET_TEMPERATURE = 0x015
    CMD_API_REBOOT = 0x016
    CMD_API_GET_BUS_VOLTAGE_CURRENT = 0x017
    CMD_API_CLEAR_ERRORS = 0x018
    CMD_API_SET_ABSOLUTE_POSITION = 0x019
    CMD_API_SET_POS_GAINS = 0x01a
    CMD_API_SET_VEL_GAINS = 0x01b
    CMD_API_GET_TORQUES = 0x01c
    CMD_API_GET_POWERS = 0x01d
    CMD_API_ENTER_DFU_MODE = 0x01f
