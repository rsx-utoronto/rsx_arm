...
def test_manual_init():
    ...
    assert manual_node.controller_input == [0., 0., 0., 0., 0., 0, 0] and [type(value) for value in manual_node.controller_input] == [
        type(value) for value in [0., 0., 0., 0., 0., 0, 0]], "controller_input is not zero!"
    goal_pos_data = Float32MultiArray()
    goal_pos_data.data = [0., 0., 0., 0., 0., 0., 0.]
    assert manual_node.goal_pos.data == goal_pos_data.data and [type(value) for value in manual_node.goal_pos.data] == [
        type(value) for value in goal_pos_data.data], "goal_pos is not zero!"
    assert manual_node.error_messages == [
        0, 0, 0, 0, 0, 0, 0], "error_messages is not zero!"
    assert manual_node.error_offsets == [
        0, 0, 0, 0, 0, 0, 0], "error_offsets is not zero!"
    assert manual_node.SPEED_LIMIT == [-0.1, 0.09, 0.15, 0.75,
                                       0.12, 0.12, 20], "SPEED_LIMIT does not match the values!"
...
def test_arm_input_sub():
    ...
    assert list(manual_node.goal_pos.data) == [
    ...
    time.sleep(0.2)
    assert manual_node.controller_input == [
        l_horizontal, l_vertical, r_vertical, r_horizontal, l1 - r1, l2 - r2, x - o]
...
def test_arm_input_sub_manual():
    ...
    expected_input = [l_horizontal, l_vertical,
                      r_vertical, r_horizontal, l1 - r1, l2 - r2, x - o]
    assert manual_node.controller_input == expected_input

    expected_goal_pos = Float32MultiArray()
    expected_goal_pos.data = list(np.array(
        expected_input) * np.array(manual_node.SPEED_LIMIT) + np.array(initial_goal_pos))
...
def test_update_pos_zero_input():
    ...
    assert out == goal, "Zero joystick should not change goal position"
