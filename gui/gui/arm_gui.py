...
class CameraFeed(QLabel):
    ...
    def update_image(self, cv_image):
        ...
        try:
            ...
            overlay[:, :, 1] = np.clip(
                overlay[:, :, 1] * 1.1, 0, 255)  # Boost green
            overlay[:, :, 0] = np.clip(
                overlay[:, :, 0] * 0.9, 0, 255)  # Reduce blue

            q_img = QImage(overlay.data, width, height,
...
class ArmGUI(Node, QWidget):
    ...
    def setup_ui(self):
        ...
        self.dist_check.stateChanged.connect(
            lambda: self.toggle_target_display())
        ...
        joints_group = self.create_group_box(
            "▸ JOINT CONFIGURATION", "#ffff00")
        joints_layout = QVBoxLayout()

        self.curr_joints_indicator = StatusIndicator(
            "CURRENT ANGLES", "#00ff00")
        self.target_joints_indicator = StatusIndicator(
            "TARGET ANGLES", "#ffaa00")
        ...
        self.limit_switches_indicator = StatusIndicator(
            "SWITCH STATUS", "#ff0066")
    ...
    def setup_ros_subscriptions(self):
        ...
        self.create_subscription(
            Float32MultiArray, "arm_curr_angles", self.on_curr_joints, 10)
        self.create_subscription(
            Float32MultiArray, "safe_arm_target_joints", self.on_target_joints, 10)

        # New subscriptions - UPDATE TOPIC NAMES AS NEEDED
        # Camera feed
        self.create_subscription(Image, "/annotated_video", self.on_camera, 10)
        self.create_subscription(
            Image, '/camera/camera/color/image_rect_raw', self.on_camera, 10)  # Optional raw feed

        # Current pose (x, y, z, roll, pitch, yaw)
        self.create_subscription(
            Float32MultiArray, "/arm_curr_pose", self.on_curr_pose, 10)

        self.create_subscription(
            TargetPosition, "/key_targets", self.on_targets, 10)

        # Limit switches status
        self.create_subscription(
            ArmStatuses, "/arm_lim_switches", self.on_limit_switches, 10)

        # Safety status
        self.create_subscription(
            ArmStatuses, "/arm_safety_status", self.on_safety_status, 10)

        # Homing status
        self.create_subscription(
            String, "/arm_homing_status", self.on_homing_status, 10)

        # Path planning execution status
        self.create_subscription(
            String, "/arm_path_planning_status", self.on_path_planning_status, 10)
    ...
    def _update_target_displays(self):
        ...
        for name in self.targets.keys():
            if name not in self.target_displays.keys():
                ...
                self.targets_layout.insertWidget(
                    self.targets_layout.count() - 1, self.target_displays[name])

            self.target_displays[name].update_data(
                self.targets[name][0].x, self.targets[name][0].y, self.targets[name][0].z, self.targets[name][1])
    ...
    def _fmt_joints(self, joints: list) -> str:
        ...
        for i, j in enumerate(joints, start=1):
            try:
                ...
                parts.append(
                    f"J{i}={angle_deg:.1f}°" if math.isfinite(jf) else f"J{i}=?")
            except Exception:
                parts.append(f"J{i}=?")
        return " | ".join(parts)
